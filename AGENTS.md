...
Python-pptx pipeline cases: `test/e2e/oracle/cases-pypptx/*.json` (178 cases: 59 text,
31 shape-adjustment, 28 zero-adjustment flowchart, 19 static 3D, 20 composite, and 21 chart).
Prefix `oracle-pypptx-{category}-{NNNN}-{slug}` where category is `text`, `shape-adj`,
`flowchart`, `shape3d`, `composite`, or `chart`. Generated by

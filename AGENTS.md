...
Python-pptx pipeline cases: `test/e2e/oracle/cases-pypptx/*.json` (123 cases: 51 text,
31 shape-adjustment, 20 composite, 21 chart). Prefix
`oracle-pypptx-{category}-{NNNN}-{slug}` where category is `text`, `shape-adj`, `composite`, or
`chart`. Generated by `test/e2e/scripts/generate_pypptx_cases.py`; `--case` accepts repeatable
exact/glob filters. Text IDs 0040-0051 are the CJK wrap/autofit/spacing interaction matrix.

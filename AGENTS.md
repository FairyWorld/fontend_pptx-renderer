...
bounded ordinary-shape outer-shadow lane, unverified reflection/glow fidelity,
...
Python-pptx pipeline cases: `test/e2e/oracle/cases-pypptx/*.json` (179 cases: 59 text,
31 shape-adjustment, 28 zero-adjustment flowchart, 1 shape-effect, 19 static 3D, 20 composite, and
21 chart).
Prefix `oracle-pypptx-{category}-{NNNN}-{slug}` where category is `text`, `shape-adj`,
`flowchart`, `shape-effect`, `shape3d`, `composite`, or `chart`. Generated by
...
`defRPr`/`fontRef` text-color precedence.
Shape3D ID 0017 pairs omitted `bevelT@prst/@w/@h` values with explicit `circle`/76200-EMU
equivalents; preserve its `assertions.equivalentSlidePairs` contract when changing bevel parsing or
lighting.
Shape3D ID 0018 crosses donut aspect ratios `0.75|1.25|2.0` with adjustments
`10000|default25000|40000`; keep the full nine-slide cross-product when changing the aspect shadow
profile so anchor interpolation is tested rather than inferred from endpoint rows.
Shape3D ID 0019 crosses square/wide/tall physical bounds with blue/white paint for the exact
`perspectiveRelaxedModerately` multi-contour cubic custom-path profile; keep all six rows and its
schema-v6 vertical-squash mutation when changing custom camera projection.
...
permits a candidate shadow to exceed native by about 17.6%. Schema-v8 bevel evidence also requires
...
non-cancelling local shadow excess `<= 0.30` of native mean shadow energy. Every salient 30°
inner/outer donut contour sector, with native mean shadow at least 3 luma, must remain `<= 1.60`
candidate/native shadow energy. Keep all four checks when changing material, light-bearing,
coordinate space, or aspect-response curves. Total shadow energy can stay equal while one sector
becomes visibly too dark and another too light. Group child-coordinate lighting is currently verified
only for the grouped donut tuple; do not generalize it to other grouped shapes without native evidence.
...
| `scripts/generate_pypptx_cases.py`      | `[--case PATTERN]`                  | Generate 123 python-pptx cases (text/shape-adj/composite/chart) |

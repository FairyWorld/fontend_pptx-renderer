...
const defaultRuntimePilotOutput = 'src/shapes/generated/ooxmlPresetGeometryPilot.ts';
const runtimePilotShapeNames = Object.freeze(['flowChartTerminator']);
...
export function buildRuntimePilotModule(ir, shapeNames = runtimePilotShapeNames) {
  ...
  const definitions = shapeNames.map((name) => {
    ...
    if (!shape) throw new Error(`OOXML runtime pilot shape is missing from source: ${name}`);
    if (shape.paths.length !== 1) {
      throw new Error(
        `OOXML runtime pilot shape must contain exactly one path until multi-path routing is supported: ${name}`,
      );
    }
    return {
      name: shape.name,
  ...
  if (new Set(shapeNames).size !== shapeNames.length) {
    throw new Error('OOXML runtime pilot shape names must be unique');
  }
  const sourceSha256 = ir.source?.presetShapeDefinitions?.sha256;
  if (!/^[a-f0-9]{64}$/.test(sourceSha256 ?? '')) {
    throw new Error('OOXML runtime pilot requires a valid preset source SHA-256');
  }
...
export const OOXML_PRESET_GEOMETRY_PILOT = ${serializedDefinitions} as const;
`;
}
...
function parseArguments(arguments_) {
  const options = {
    ...
    runtimeOutput: defaultRuntimePilotOutput,
...
export async function generatePresetShapeCatalog({
  ...
  const runtimeGenerated = buildRuntimePilotModule(geometryIr);
  ...
  const runtimePersisted = await writeOrCheckGeneratedCatalog({
    artifactLabel: 'OOXML geometry runtime pilot',
    check,
